(ns marketentry.marketentryllm
  ...
  the Andorra (AND) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "contract-type=" (:contract-type e)
                        " contract-value=" (:contract-value e)
                        " urgent?=" (:urgent? e)
                        " direct-contracting-claim?=" (:direct-contracting-claim? e)
                        " nrt-verified?=" (:nrt-verified? e)
