(ns marketentry.store
  "SSoT for the Andorra (AND) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:contract-type` / `:contract-value` / `:urgent?` /
  `:direct-contracting-claim?` are ground truth for the
  `direct-contracting-threshold-mismatch` flagship check (Llei 14/2022
  Art. 30.1 direct-contracting thresholds); `:requires-nrt?` /
  `:nrt-verified?` are ground truth for the conditional NRT check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Vall del Nord Serveis SL" :portal "contractacio.govern.ad"
             ...
             :requires-nrt? true :nrt-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AND" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "contractacio.govern.ad"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :contract-type :services-supplies :contract-value 12000 :urgent? false
             :direct-contracting-claim? true
    ...
    "eng-3" {:id "eng-3" :operator "Pirineus Construccions SA" :portal "contractacio.govern.ad"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "AND" :status :intake}
    "eng-4" {:id "eng-4" :operator "Escaldes Digital SLU" :portal "contractacio.govern.ad"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "AND" :status :intake}
    "eng-5" {:id "eng-5" :operator "Andorra la Vella Logistics SL" :portal "contractacio.govern.ad"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :contract-type :services-supplies :contract-value 12000 :urgent? false
             :direct-contracting-claim? true
             :requires-nrt? true :nrt-verified? false
             :drafted? false :submitted? false
             :jurisdiction "AND" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               contract-type contract-value urgent? direct-contracting-claim?
                               requires-nrt? nrt-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? urgent?)                       (assoc :engagement/urgent? urgent?)
    (some? direct-contracting-claim?)     (assoc :engagement/direct-contracting-claim? direct-contracting-claim?)
    (some? requires-nrt?)                 (assoc :engagement/requires-nrt? requires-nrt?)
    (some? nrt-verified?)                 (assoc :engagement/nrt-verified? nrt-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/contract-type :engagement/contract-value :engagement/urgent? :engagement/direct-contracting-claim?
   :engagement/requires-nrt? :engagement/nrt-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :urgent? (boolean (:engagement/urgent? m))
     :direct-contracting-claim? (boolean (:engagement/direct-contracting-claim? m))
     :requires-nrt? (boolean (:engagement/requires-nrt? m))
     :nrt-verified? (boolean (:engagement/nrt-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-and"})
