...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (AND, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up direct-contracting-threshold-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (direct-contracting-threshold-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up nrt-unverified) ==")
    ...
    (println "== filing/submit eng-5 (nrt-unverified -> HARD hold) ==")
