  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Vall del Nord Serveis SL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Vall del Nord Serveis SL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest direct-contracting-threshold-mismatch-is-held-and-unoverridable
  (testing "claimed direct-contracting eligibility doesn't match the independently recomputed Art. 30.1 thresholds -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:direct-contracting-threshold-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest nrt-unverified-is-held-and-unoverridable
  (testing "unverified NRT (Número de Registre Tributari) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:nrt-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
